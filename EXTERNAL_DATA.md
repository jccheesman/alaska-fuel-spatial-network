...
The honest inventory: what is committed, what is regenerable-only (i.e. **not
on disk in a fresh clone and possibly on no machine at all right now**), and
...
| Path | Size | What |
|---|---|---|
| `inputs/bulk_fuel_data.zip` | 0.4 MB | AEA facilities inventory + delivery-method shapefile (repacked clean; facilities CSV normalized to LF) |
...
| `inputs/region_and_census_data.zip` | 6.7 MB | AEA regions + TIGER state boundary |
...
|---|---|---|---|
| `inputs/gee_exports/AK_Stack_150m.zip` | ~5 GB | **Located (2026-08-06): the owner's machine** — the June narrow-grid export generation, kept as provenance. The CANONICAL wide-grid layers live in the owner's `friction_rasters` working set. | To regenerate from nothing: run `source_scripts/friction_surface/friction_preprocessing/gee_friction_layer_multi_data_processing.js` in the GEE Code Editor (use the 28,001-column extended grid). |
...
end-to-end and all gates passed (docs/TEST_LOG.md). A machine WITHOUT the
rasters still cannot run the friction half — this file tells you where the
data lives and how to regenerate it.
