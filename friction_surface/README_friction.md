...
`./friction_surface/friction_inputs`).
...
`friction_preprocessing/` (sibling of `friction_inputs/`). GEE Code Editor
scripts handle the slope/LULC/permafrost/sea-ice exports; the ArcGIS Pro
pipeline `friction_preprocessing/river_ice_full_pipeline.py` produces the
12 monthly `river_ice/*.tif`. See `friction_preprocessing/README.md` for
the pipeline and `friction_inputs/README.md` for the per-file export
conventions.

## Outputs

- **Friction rasters** written to
  `./friction_surface/friction_outputs/friction_stack/`: 12 monthly
...
- Overland over water → NoData (except road-bridge pixels burned in from
  `road_mask`, which carry `ROAD_BRIDGE_FRICTION` to keep the network
  connected across river crossings).
...
- Barge over land **not on the waterway network** → NoData. Barge
  ...
  the rasterized waterway network (`outputs/waterway_mask_150m.tif`)
...
mode: during `ICE_ROAD_SEASON_MONTHS = {1, 2, 3}` the ice-road mask
is burned into the overland friction surface at
`max(ROAD_FRICTION, slope_friction) * ICEROAD_TIME_PENALTY * permafrost_mod`
— symmetric with the highway burn-in, with LULC intentionally dropped
because an ice road is an engineered packed-snow surface (the
underlying wet-tundra / shrub class shouldn't add a substrate penalty
on top of the 2× seasonal slowdown). Out of season the burn-in is
skipped and those pixels revert to underlying terrain cost (off-road
tundra, effectively impassable), so the routing graph naturally drops
the seasonal connectivity without special-casing.
...
- `inputs/networks/ice_roads_150m_3338/Ice_Roads.shp` — merged ice-road
  LineString network (NOS + SIRA + Alaska Trails). Rasterized to a 150 m
  binary mask via `build_corridor_masks.py`; the overland surface
  consumes this mask in-season.
- `inputs/Fuel_Delivery_Method/Fuel_Delivery_Method.shp` — per-community
...
On ice-road mask pixels burned into overland during the in-season
months, friction is computed as
`max(ROAD_FRICTION, slope_friction) * ICEROAD_TIME_PENALTY * permafrost_mod`,
with `ICEROAD_TIME_PENALTY = 2.0`. Under travel-time semantics this
means a flat, non-permafrost ice-road pixel takes ~2× as long to
traverse as a highway pixel. LULC is dropped (mirrors highway burn-in)
because the engineered packed-snow surface, not the underlying tundra
class, is what the truck contacts; slope rides through via `max()` so
any grade is still penalized; permafrost is retained for
...
extension), so no overland↔ice_road handoff is required. The previous
`(overland, ice_road)` transfer-fee entries in `friction_costs.py` are
defunct and slated for cleanup as part of the cost-rate rewiring.
...
|---|---|---|
| `RASTER_DIR` | Preprocessed input rasters (`slope.tif`, `lulc.tif`, `permafrost.tif`, `sea_ice/`, `river_ice/`) | `friction_surface/friction_inputs` |
| `FRICTION_DIR` | Friction-stack output dir | `friction_surface/friction_outputs/friction_stack` |
| `NETWORK_DIR` | Network vector shapefiles | `inputs/networks` |
| `INPUTS_DIR` | Ice-road / fuel-delivery datasets | `inputs` |
| `OUTPUTS_DIR` | Rasterized corridor masks | `outputs` |
| `VECTOR_DIR` | Misc vector data | `vectors` |
| `FRICTION_NO_CHDIR` | If set (e.g. `=1`), suppresses the import-time `chdir` into the repo root | *(unset — chdir happens)* |

On import, `friction_paths` also `chdir`s into the repo root so that
sibling scripts using bare relative paths (`./outputs`) keep working. The
anchored defaults above do **not** rely on that chdir; set
`FRICTION_NO_CHDIR=1` when embedding this package in another tool that
manages its own working directory.

Dependencies for building the surface stack alone (no DuckDB/WBT) are pinned
in `friction_surface/requirements-friction.txt`; the repo-root
`requirements.txt` remains the source of truth for the end-to-end run.
...
| File | Purpose |
|---|---|
| `run_friction_pipeline.py` | Friction-build entry point. Input validation, surface build, summary printout. Repo-bound (uses root `pipeline.py`). |
| `check_grid_exports.py` | One-off gate: verify re-exported inputs sit on the canonical full-Alaska grid. |
| `viz/*`, `qa/*`, `friction_preprocessing/*` | Diagnostic plots, post-build QA, and upstream data prep (see **Subdirectories**). |
...
| `friction_inputs/` | Inputs consumed by this stage: `lulc.tif`, `slope.tif`, `permafrost.tif`, and the monthly `sea_ice/` and `river_ice/` stacks. See its README for the export conventions. |
| `friction_outputs/` | Generated friction stack in `friction_stack/` — 14 files (`overland.tif`, `barge_{MM}.tif`, `road_base.tif`) backing 24 logical `(mode, month)` entries — plus QA artifacts. |
| `viz/` | Visualization generators — `plot_friction_stack.py` (3×4 monthly raster grid PNG), `generate_grid_schema.py` (cell-by-cell schematic PNG), `generate_pipeline_diagram.py` (two-page PDF). The generated `friction_pipeline_diagram.pdf` lives here. |
...
| `tests/` | Pytest suite — `test_friction_surface.py`. Run with `pytest friction_surface/tests/test_friction_surface.py -v` from the project root. |
