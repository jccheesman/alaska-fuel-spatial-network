...
# ===========================================================================
# Reads inputs/literature_sources_for_agent_analysis/market_data/
# Fuel_Delivery_Method.csv (path in friction_paths.FUEL_DELIVERY_METHOD_CSV),
...
def load_ice_road_communities(as_of: _date | None = None) -> frozenset[str]:
    """Return the set of community names served by ice road as of a given date.

    Reads the CSV form of Fuel_Delivery_Method (faster than the shapefile),
    groups by CommunityName, takes the most recent AsOfDate <= as_of per
    community, and filters to rows where Fuel_Delivery_Method == 'Ice Road'.
    ...
    from .friction_paths import FUEL_DELIVERY_METHOD_CSV

    df = pd.read_csv(FUEL_DELIVERY_METHOD_CSV)
