...
Output: friction_surface/viz/friction_pipeline_diagram.pdf
Re-run after structural changes to keep the diagram current.

    python -m friction_surface.viz.generate_pipeline_diagram
"""
...
def render_flowchart(pdf: PdfPages) -> None:
    ...
    fig.text(
        0.5, 0.02,
        "Regenerable artifact — run  python -m friction_surface.viz.generate_pipeline_diagram  "
...
FILE_CARDS = [
    ...
    },
    {
        ...
        "bullets": [
            ...
            "Corridor masks: ICE_ROAD_MASK_TIF, ROAD_MASK_TIF,",
            "    WATERWAY_MASK_TIF",
            "Networks: roads, waterways, airports, ports, flights",
            "Ice-road: ICE_ROADS_SHP, FUEL_DELIVERY_METHOD_*",
            "Chdirs to project root (opt-out: FRICTION_NO_CHDIR)",
        ],
    },
    {
        "name": "friction_io.py",
