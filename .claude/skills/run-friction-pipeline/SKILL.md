...
3. **A rebuild overwrites the friction stack.** `--rebuild` runs the
   surface build (workflow 01 stage 02) — it does NOT touch
   `outputs/fuel_network.duckdb`; edge weighting is workflow 03's job and
   must be rerun separately after a surface change. Validation without
   `--rebuild` is read-only.
...
# rebuild first, then validate (writes outputs/fuel_network.duckdb)
...
3. **`run_friction_pipeline`** *(only with `--rebuild`)* — builds the 14
   ...
   backing the 24 logical (mode, month) entries). Surface-only: rerun
   workflow 03 (weight + assemble) afterwards to refresh the DB.
...
2. **Decide rebuild scope.** Friction-value or input-raster change → needs a
   surface rebuild (`--rebuild`), then a workflow-03 rerun (weight +
   assemble). Cost-rate-only change → surfaces are unchanged; rerun only
   workflow 03 stage 04 (`04_assemble_weighted_graph.py`). (The old
   `--skip-surfaces` flag never existed — that was a documentation bug.)
