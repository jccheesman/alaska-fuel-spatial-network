...
Chain (each step gates the next unless --keep-going):
  ...
  3. [--rebuild only]     — run_friction_pipeline: rebuild the 14 friction
                            surfaces (SURFACE-ONLY — it does not touch the
                            DuckDB; edge weighting lives in workflow 03)
...
step 3, which rebuilds the friction stack.
...
INTERP = {
    ...
    "rebuild": {
        None: (
            ...
            "isolate. A write error against outputs/fuel_network.duckdb usually means "
...
def main() -> int:
    ...
    ap.add_argument("--rebuild", action="store_true",
                    help="Insert the build+edge+DuckDB pipeline (MUTATES outputs/fuel_network.duckdb).")
