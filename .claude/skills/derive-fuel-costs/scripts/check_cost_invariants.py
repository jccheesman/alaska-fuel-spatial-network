...
Run after ANY change to friction_surface/friction_costs.py rates, fees, or
mode metadata:
...
Checks (FAIL = exit 1; WARN/SKIP never fail the run):
  ...
  6. Graph fee coverage: every live Transfer edge in fuel_network.duckdb
...
def check_graph_coverage():
    db = REPO_ROOT / "fuel_network.duckdb"
    if not db.exists():
        record("SKIP", "graph.fee_coverage", "fuel_network.duckdb not found")
        return
    try:
        import duckdb
