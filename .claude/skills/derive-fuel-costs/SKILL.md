...
6. **No costless edge.** Every modal boundary that can appear as a Transfer
   ...
   `friction_costs._lookup_fee` hard-errors on a missing pair —
...
- `FEE_MODE` (edge_class → fee vocabulary) and `infer_transfer_fees()`
  (prices live Transfer edges) also live HERE in friction_costs.py —
  `workflows/03_multimodal_join/04_assemble_weighted_graph.py` imports them
  (the old verbatim copy there was deleted in the two-repo merge)
...
- `scripts/check_cost_invariants.py` — mechanical audit: rates in ranges,
  ...
  test, live-graph fee coverage against `outputs/fuel_network.duckdb`, workbook
...
7. **Refresh downstream artifacts**: rerun
   ...
   `workflows/03_multimodal_join/04_assemble_weighted_graph.py` (edge_costs), and `plot_cost_equation.py`
...
- `run-friction-pipeline` skill — the build-and-validate phase; after a
  cost change, surfaces are unchanged — rerun workflow 03 stage 04
  (`04_assemble_weighted_graph.py`) to reprice edge_costs, then validate.
