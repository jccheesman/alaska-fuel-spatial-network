...
1. **Single source of truth.** All friction multipliers and thresholds live in
   `friction_surface/friction_config.py`. Never hardcode a friction value in a
...
- Adding corridor masks (`build_corridor_masks.py` already handles that)

## Where the canonical values live

- `friction_surface/friction_config.py` — all multipliers, thresholds, constants
