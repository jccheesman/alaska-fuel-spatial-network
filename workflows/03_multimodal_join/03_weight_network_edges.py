...
The final_network topology is fixed (92,978 edges with real geometries), so
...
92,978 edges x 12 months = 1,115,736 rows.
...
def load_edge_class(db_path: str | Path, n_edges: int) -> np.ndarray:
    """Read the edge_class column stage 02 persisted, in edge_id order.

    edge_class is derived exactly once (02_load_final_network.py's
    derive_edge_class); consuming it here keeps the IceRoad/weld rule from
    being maintained in two places. Hard error if stage 02 hasn't run or
    the row count disagrees with the shapefile.
    """
    if not Path(db_path).exists():
        raise FileNotFoundError(
            f"{db_path} not found — run workflows/03_multimodal_join/"
            "02_load_final_network.py first (it writes network_edges with "
            "the edge_class column this script consumes)."
        )
    con = duckdb.connect(str(db_path), read_only=True)
    try:
        tables = {r[0] for r in con.execute("SHOW TABLES").fetchall()}
        if "network_edges" not in tables:
            raise ValueError(
                f"{db_path} has no network_edges table — run "
                "02_load_final_network.py first."
            )
        rows = con.execute(
            "SELECT edge_class FROM network_edges ORDER BY edge_id"
        ).fetchall()
    finally:
        con.close()
    edge_class = np.array([r[0] for r in rows])
    if len(edge_class) != n_edges:
        raise ValueError(
            f"network_edges has {len(edge_class)} rows but the shapefile has "
            f"{n_edges} — rerun 02_load_final_network.py against the current "
            "export (edge_id = row order must match)."
        )
    return edge_class
