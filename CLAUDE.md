...
| `04_build_network.py` | validate_profile + mmnet stages 01→04 | `outputs/02_network_build/{output,reports}` | `profile.yaml` (THE config surface) | Region-as-data: improve the model by editing the profile, not the engine. **All Barge-mode layers are concatenated** into the waterway (was single-layer), so `manual_barge` isn't dropped |
