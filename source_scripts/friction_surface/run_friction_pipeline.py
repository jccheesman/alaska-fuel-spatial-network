...
def main() -> None:
    ...
    parser.add_argument(
        "--output-dir",
        default=None,
        help="Where to write friction TIFs (defaults to FRICTION_DIR env var or outputs/01_friction_build/friction_stack).",
    )
    parser.add_argument(
        "--allow-missing-waterway-mask",
        action="store_true",
        help="Build barge surfaces WITHOUT the waterway corridor mask "
             "(synthetic/research runs only — severs ~18%% of waterway "
             "edges; the default is a hard error).",
    )
    ...
    from .friction_paths import get_friction_output_dir, get_raster_dir
    input_dir = Path(args.input_dir) if args.input_dir else Path(get_raster_dir())
    ...
    friction_stack = write_friction_stack(
        input_dir, output_dir,
        require_waterway_mask=not args.allow_missing_waterway_mask,
    )
