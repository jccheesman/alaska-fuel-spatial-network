...
Run from the project root:
    pytest tests/test_friction_surface.py -v
"""
...
def test_ice_split_invariant_at_equal_thresholds():
    ...
    """
    import pytest
    from friction_surface.friction_config import SEA_ICE_THRESHOLD, RIVER_ICE_THRESHOLD
...
def test_overland_water_pixels_are_nodata():
    ...
    from friction_surface.friction_config import FRICTION_NODATA
    sb, wm, pm, ni, _, _ = _bridge_test_inputs()
...
def _make_stack_inputs(input_dir, iced_month: int = 1) -> None:
    ...
    """
    from friction_surface.friction_config import LULC_WATER_CLASS
...
def test_write_friction_stack_end_to_end(tmp_path, monkeypatch):
    ...
    path with require_waterway_mask=False — the explicit synthetic-run
    escape hatch — so barge uses the LULC-water fallback.
    """
...
def test_missing_waterway_mask_is_a_hard_error_by_default(tmp_path, monkeypatch):
    """Public-pipeline guarantee: a maskless barge build FAILS, never warns.
    ...
    monkeypatch.delenv("FRICTION_ALLOW_MISSING_WATERWAY_MASK", raising=False)
    ...
    with pytest.raises(FileNotFoundError, match="waterway corridor mask is REQUIRED"):
        write_friction_stack(input_dir, output_dir, preflight=False)
