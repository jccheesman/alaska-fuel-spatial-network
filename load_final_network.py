...
`edge_class` disambiguates the colleague's `Bridge` type (an mmnet topology
...
becomes `IceRoadWeld` (seasonal, Jan-Mar), remaining Bridge edges become
`Weld`, every other type maps through unchanged.
...
FINAL_NETWORK_DIR = Path("final_network")
...
def derive_edge_class(edge_type: pd.Series, source: pd.Series) -> pd.Series:
    """Disambiguated edge class (colleague's `Bridge` = topology weld).

    Bridge + ice-road provenance -> IceRoadWeld (seasonal, treated as
    IceRoad by the weighting); other Bridge -> Weld (year-round stitch);
    ...
    edge_class[ice_weld] = "IceRoadWeld"
    return edge_class
